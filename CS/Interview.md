...
15. 브라우저에서 주소를 입력 하였을때 해당 주소로 접속하게되는 과정을 osi 7 레이어 순서대로 설명 해보세요.

브라우저에서는 http request를 만들고 해당 내용을 암호화, 인코딩 작업을 한 후 OS에게 요청을 전달합니다. 이때 OS에 송신을 의뢰할 때에는 도메인 명이 아니라 IP 주소를 알아야하므로 DNS서버를 조회합니다.

프로토콜 스택(운영체제에 내장된 네트워크 제어용 소프트웨어)이 브라우저로부터 메세지를 받습니다. 포트번호와 같은 제어 정보를 덧붙여서 데이터를 Segment단위로 나눕니다.

라우팅 알고리즘을 통해 목적지까지의 최단경로를 파악하고 데이터를 패킷 단위로 나누어 전송합니다.
